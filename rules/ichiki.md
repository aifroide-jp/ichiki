...
> macOS の Finder だけが隠すが、比較は `ichiki diff` のレポートを見るので困らない。
...
> 実測: `ichiki diff` が出す比較レポート `docs/visual/index.html` を
...
## 成果物
- テスト仕様書（Markdown、1ページ1テストケース）を自動出力する（`ichiki testspec`）。
  出力する種別はページ別に6つ:
