...
## 共通領域の扱い（Phase 1 への申し送り）
- Phase 0 の scan は nav・footer をページごとに列挙する（全ページ重複して出力される）。
- Phase 1 で WPテーマ化する際、同一の nav・footer は共通パーツ（header.php / footer.php・カスタムメニュー）に名寄せし、ページ単位のACFフィールドにはしない。
- footer など同名セクションがページを跨いで重複する場合（例: footer_text が複数ページに出る）、共通フィールドに寄せるか、ページ別フィールドグループとして分離するかを Phase 1 で決める。Phase 0 は重複したまま列挙する仕様とする。
