...
ルートに構造化前のページ、別ディレクトリに構造化済みのページが並び、
同じ相対パスのページが2箇所に存在した（画像も数十MB重複）。
`.ichiki.json` の `mockup` を書き換えるまで、引数無しの `ichiki gate` が落ちていた。
名前も効かない。`mockup-real` は「何の real か」を説明していなかった。

### モックでないものを走査から外す

ルートがモックになると、成果物の置き場所まで走査される。
実測: `ichiki diff` が出す比較レポート `docs/visual/index.html` をモックのページとして拾い、
lint が242件のエラーを出した。

- 隠しディレクトリと `node_modules` は元から外れる
- それ以外は既定で `docs` と `scripts` を外す
- 足りなければ `.ichiki.json` に宣言する
...
合意デザインは記録として残すが、置き場所と名前を固定する。

```
/（ルート）              いまのモック。お客様と合意するもの
.ichiki/mockup-before/   合意デザイン。記録用
```
...
構造化は現実的な作業量である。推測でクラス名から構造を当てる仕組みを常設せず、
...
CF7 の死にフィールド）は、一部のページを WP 化した時点で見つかった。
...
- 本番リリース手順書（Markdown、エックスサーバー向け）を自動出力する（`ichiki release`）。
